...
-npx @react-native-community/cli init Tankold_MQTT --package-name "com.tankold_mqtt"


-npx @react-native-community/cli init App  renombrar: -npm install react-native-rename -g -react-native-rename "Tankold"
-npm install
...
type MQTTClient = InstanceType<typeof Paho.Client>;
...
const MQTTExample = () => {
  ...
  const connect = useCallback(() => {
    if (client) {
      client.connect({
        onSuccess: () => {
          setStatus('Connected');
          client.subscribe('testtopic/react-native');
        },
        onFailure: (err: any) => {
  ...
  const publishMessage = () => {
    if (client && client.isConnected()) {
      const message = new (require('react-native-mqtt').Paho.Message)(messageToSend || 'Hello from React Native!');
...
copiar src de app-v0.5.7

crear src/@types/react-native-mqtt/index.d.ts

declare namespace ReactNativeMqtt {
    interface Message {
      new(payload: string): Message;
      payloadString: string;
      destinationName: string;
    }
  
    interface Client {
      new(host: string, port: number, clientId: string): Client;
  ...
  declare module 'react-native-mqtt' {
    export const Paho: ReactNativeMqtt.PahoInterface;
  }
...
pues volvió a valer verga porque al instalar cualquier librería ya no se compila el proyecto
enconte un ejemplo con MQTT.js sin instalar nada solo abrir todo el archivo del proyecto si se conecto pero solo a mosquito.org al hivemq creo que no se conecto por el firewall del emulador de android yo creo que compilando ese proyecto pero en apk si va funcionar copie todo al app-v5.8 y no se ejecuto entonces ahora voy a usar Claude code para hacerlo según es mejor
a ver que pedo dijo el ciego
